...
public static class D7TheTreacheryOfWhales
{
    public static int SolvePart1(string[]? customInput = null)
    {
        var positions = Array
            .ConvertAll(customInput ?? PuzzleInput.GetFromFileAsStrings("PuzzleInputD7.txt").First().Split(','),
                int.Parse).ToList();
        ...
        for (var i = positions.Min(); i <= positions.Max(); i++)
        {
            ...
            foreach (var crab in positions) count += Math.Abs(crab - i);
