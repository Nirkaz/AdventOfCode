...
public static class D5HydrothermalVenture
{
    public static int SolvePart1(string[]? customInput = null)
    {
        ...
        for (var i = 0; i < inputRaw.Length; i++)
        {
            ...
            if (fromList[i].Item2 != toList[i].Item2) continue; // horizontal = 4
            {
    ...
    public static int SolvePart2(string[]? customInput = null)
    {
        ...
        for (var i = 0; i < inputRaw.Length; i++)
        {
            ...
            if (Math.Abs(fromList[i].Item1 - toList[i].Item1) !=
                Math.Abs(fromList[i].Item2 - toList[i].Item2)) continue; // Diagonal
            if (fromList[i].Item1 < toList[i].Item1)
            {
                if (fromList[i].Item2 < toList[i].Item2)
                    for (int x = fromList[i].Item1, y = fromList[i].Item2;
                         x <= toList[i].Item1 && y <= toList[i].Item2;
                         x++, y++)
                        grid[x, y]++;
                else
                    for (int x = fromList[i].Item1, y = fromList[i].Item2;
