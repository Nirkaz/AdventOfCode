...
public static class D4GiantSquid
{
    public static int SolvePart1(string[]? customInput = null)
    {
        ...
        for (var i = 2; i < inputRaw.Length; i += 6)
        {
            ...
            for (var j = 0; j < 5; j++)
            {
                var line = Array.ConvertAll(inputRaw[i + j].Split().Where(s => !string.IsNullOrWhiteSpace(s)).ToArray(),
                    int.Parse);
        ...
        foreach (var draw in drawNumbers)
        {
            ...
            foreach (var board in boards)
            {
                ...
                if (board[0, column] != 5 && board[row, 0] != 5) continue;
                winner = true;
